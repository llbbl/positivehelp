...
import { generateSEOMetadata, generateMessageDescription, cleanTextForMeta, generateStructuredData } from '@/lib/seo';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!message) {
    return generateSEOMetadata({
      ...
      description: "The requested positive message could not be found. Discover other uplifting content on positive.help",
      noIndex: true,
    });
  }

  const cleanMessage = cleanTextForMeta(message.text);
  const authorName = message.authors && message.authors.length > 0 ? message.authors[0].name : undefined;
  ...
  return generateSEOMetadata({
    ...
    keywords: ['positive message', 'inspiration', 'motivation', 'uplifting quote', 'good vibes'],
  });
}
...
export default async function MessagePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const articleStructuredData = generateStructuredData('article', {
      ...
      description: generateMessageDescription(cleanTextForMeta(message.text), authorName),
