...
if (process.env.NEXT_PUBLIC_VERCEL_BRANCH_URL && process.env.VERCEL_ENV === 'preview') {
  appUrl = `https://${process.env.NEXT_PUBLIC_VERCEL_BRANCH_URL}`;
} else if (process.env.NEXT_PUBLIC_VERCEL_PROJECT_PRODUCTION_URL && process.env.VERCEL_ENV === 'production') {
