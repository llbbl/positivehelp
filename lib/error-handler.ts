...
export function validateInteger(value: unknown, fieldName: string, min?: number): number {
  const parsed = typeof value === 'string' ? parseInt(value, 10) : Number(value);
  
  if (isNaN(parsed)) {
    throw new APIError(`${fieldName} must be a valid integer`, 400, 'VALIDATION_ERROR');
  }
