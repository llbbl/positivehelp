...
export const messageSchemas = {
  ...
  create: z.object({
    text: z.string()
      ...
      .refine(text => text.length >= 3, "Message must be at least 3 characters after sanitization") 
      .refine(text => text.length <= 1000, "Message cannot exceed 1000 characters")
      .refine(text => text.length > 0, "Message cannot be empty after sanitization"),
    author: z.string()
      .trim()
      .transform(val => val === "" ? undefined : sanitizeContent(val)) // Sanitize author too
      .optional()
      .refine(val => !val || val.length <= 100, "Author name cannot exceed 100 characters after sanitization"),
